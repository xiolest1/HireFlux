...
for (const route of routes) {
  test(`${route.name} is responsive and accessible`, async ({
    ...
    if (route.name === "landing") {
      ...
      const expectedFamily = ["narrow-320", "mobile-390"].includes(testInfo.project.name) ? "a" : "c";
      await expect(page.locator("[data-connected-story]")).toHaveAttribute("data-connected-family", expectedFamily);
