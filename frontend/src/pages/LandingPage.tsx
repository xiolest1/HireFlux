...
export function LandingPage() {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#f7f8fa] text-slate-950 dark:bg-slate-950 dark:text-slate-50">
      ...
      <header className="relative z-20 border-b border-slate-200/80 bg-white/85 backdrop-blur dark:border-slate-800 dark:bg-slate-950/80">
        <div className="mx-auto flex min-h-18 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3"><span className="hidden rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold uppercase tracking-[0.14em] text-slate-600 dark:border-slate-700 dark:bg-slate-900 sm:inline-flex">Recruiter demo</span><ThemeToggle /></div>
        </div>
      </header>

      <main id="landing-main">
        <section className="relative isolate">
          <div aria-hidden="true" className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_16%_12%,rgba(34,211,238,0.16),transparent_28rem),radial-gradient(circle_at_82%_4%,rgba(139,92,246,0.16),transparent_30rem)] dark:bg-[radial-gradient(circle_at_16%_12%,rgba(34,211,238,0.12),transparent_28rem),radial-gradient(circle_at_82%_4%,rgba(139,92,246,0.13),transparent_30rem)]" />
          <div className="mx-auto grid max-w-7xl gap-14 px-4 py-16 sm:px-6 sm:py-24 lg:grid-cols-[1.02fr_0.98fr] lg:items-center lg:px-8 lg:py-28">
            <div>
              <p className="inline-flex items-center gap-2 rounded-full border border-brand-100 bg-white/70 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.16em] text-brand-700 shadow-sm backdrop-blur dark:border-cyan-900 dark:bg-slate-900/70"><Sparkles aria-hidden="true" className="size-3.5" />A focused job-search workspace</p>
              ...
              {notice ? <div className="mt-7 max-w-xl rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-950" role="status">{notice}</div> : null}
            ...
            <figure className="relative mx-auto w-full max-w-xl lg:mx-0" aria-label="Decorative preview of the HireFlux applications workspace">
              ...
              <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_28px_90px_rgba(15,23,42,0.16)] dark:border-slate-700 dark:bg-slate-900 dark:shadow-[0_32px_100px_rgba(0,0,0,0.45)]">
              <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-700"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Demo workspace</p><p className="mt-1 font-bold text-slate-950 dark:text-white">Needs attention</p></div><span className="rounded-full bg-danger-soft px-3 py-1.5 text-xs font-bold text-danger">3 actions</span></div>
                <div className="space-y-3 bg-slate-50/80 p-4 sm:p-5 dark:bg-slate-950/60">
                  {[
                    ...
                    <div key={company} className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900"><div aria-hidden="true" className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-slate-900 text-sm font-black text-white dark:bg-slate-700">{company.slice(0, 1)}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-slate-950 dark:text-white">{role}</p><p className="mt-0.5 truncate text-xs text-slate-500 dark:text-slate-400">{company}</p><p className="mt-2 inline-flex items-center gap-1 text-[0.68rem] font-semibold text-slate-500"><Clock3 aria-hidden="true" className="size-3" />{next}</p></div><span className={`self-start rounded-full px-2.5 py-1 text-xs font-bold ${tone === "emerald" ? "bg-emerald-100 text-emerald-800" : tone === "violet" ? "bg-violet-100 text-violet-800" : "bg-sky-100 text-sky-800"}`}>{stage}</span></div>
                  ))}
                </div>
              </div>
        ...
        <section className="border-y border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900/80" aria-label="Demo guarantees">
          <div className="mx-auto grid max-w-7xl gap-px bg-slate-200 dark:bg-slate-800 sm:grid-cols-3">
            {[
            ...
            ].map(([Icon, title, description]) => <article key={String(title)} className="bg-white px-6 py-9 dark:bg-slate-900 lg:px-8"><Icon aria-hidden="true" className="size-5 text-brand-700" /><h2 className="mt-4 font-bold text-slate-950 dark:text-white">{String(title)}</h2><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{String(description)}</p></article>)}
          </div>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-24 lg:px-8" aria-labelledby="proof-title">
          ...
          ].map(([number, title, description]) => <li key={number} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-panel dark:border-slate-800 dark:bg-slate-900"><span className="text-sm font-black text-brand-700">{number}</span><h3 className="mt-6 text-lg font-bold text-slate-950 dark:text-white">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{description}</p></li>)}</ol>
        </section>
      ...
      <footer className="mx-auto flex max-w-7xl flex-col gap-2 border-t border-slate-200 px-4 py-8 text-sm text-slate-600 dark:border-slate-800 dark:text-slate-400 sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8"><p>HireFlux · Portfolio demonstration</p><p>Temporary workspaces expire automatically after 24 hours.</p></footer>
    </div>
  );
}
