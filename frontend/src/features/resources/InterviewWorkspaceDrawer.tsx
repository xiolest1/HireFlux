...
export function InterviewWorkspaceDrawer({
  ...
  const completedCount = guidance.checklist_items.filter((item) => completed.has(item.item_id)).length;
  ...
  function addSuggestedQuestion(question: string) {
    if (
      normalizedQuestions.some((value) => value.toLocaleLowerCase() === question.toLocaleLowerCase()) ||
    ...
    if (emptyIndex >= 0) {
      ...
      setDraft({ ...draft, candidate_questions: [...draft.candidate_questions, question] });
    }
  }
  ...
  return (
    <Drawer
      open
      onClose={onClose}
      title={interview.status === "COMPLETED" ? "Interview debrief" : "Interview preparation"}
      ...
      footer={
        <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          ...
          {interview.status === "COMPLETED" ? (
            <>
              <Button type="button" variant="secondary" disabled={mutation.isPending} onClick={() => void save(false)}>
                Save draft
              </Button>
              <Button type="button" disabled={!canCompleteDebrief || mutation.isPending} onClick={() => void save(true)}>
                {interview.debrief_completed_at ? "Save debrief" : "Complete debrief"}
              </Button>
            </>
          ) : (
            <Button type="button" disabled={mutation.isPending} onClick={() => void save(false)}>
    ...
    >
      <div className="space-y-7">
        {mutation.error ? <ErrorPanel compact title="Workspace could not be saved" error={mutation.error} /> : null}
        ...
        <section aria-labelledby="readiness-title" className="rounded-2xl border border-accent/20 bg-accent-soft p-4">
          ...
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-surface">
            <div
              ...
              style={{ width: `${(completedCount / guidance.readiness.total_steps) * 100}%` }}
            />
          </div>
          <p className="mt-3 text-xs leading-5 text-ink-muted">
            Readiness is simply the checklist below—there is no hidden score or AI judgment.
          </p>
        </section>

        <section aria-labelledby="focus-prompts-title">
          <h3 id="focus-prompts-title" className="font-bold text-ink">Focus prompts</h3>
          <ul className="mt-3 space-y-2">
            {guidance.focus_prompts.map((prompt) => (
              <li key={prompt} className="flex items-start gap-2 text-sm leading-6 text-ink-muted">
                <CircleHelp aria-hidden="true" className="mt-1 size-4 shrink-0 text-violet" />
                {prompt}
              </li>
            ))}
          </ul>
        ...
        <section aria-labelledby="checklist-title">
          ...
          <div className="mt-3 space-y-2">
            {guidance.checklist_items.map((item) => (
              <div key={item.item_id} className="flex items-start gap-3 rounded-xl border border-line bg-surface-raised p-3 hover:border-accent/40">
                <input
        ...
        <section aria-labelledby="stories-title">
          <label id="stories-title" htmlFor="interview-preparation-notes" className="font-bold text-ink">
            Evidence stories and preparation notes
          </label>
          <p className="mt-1 text-xs leading-5 text-ink-muted">
            Capture the situation, your decision, measurable impact, and what you learned.
          </p>
          <textarea
            ...
            onChange={(event) => setDraft({ ...draft, preparation_notes: event.target.value })}
        ...
        <section aria-labelledby="candidate-questions-title">
          <div className="flex items-end justify-between gap-3">
            <div>
              <h3 id="candidate-questions-title" className="font-bold text-ink">Questions you want answered</h3>
              <p className="mt-1 text-xs text-ink-muted">Prepare up to eight. Two are required to complete the question step.</p>
            </div>
            {draft.candidate_questions.length < 8 ? (
              <button type="button" className="inline-flex min-h-10 items-center gap-1.5 rounded-lg px-3 text-sm font-semibold text-accent hover:bg-accent-soft" onClick={() => setDraft({ ...draft, candidate_questions: [...draft.candidate_questions, ""] })}>
          ...
          <div className="mt-3 space-y-2">
            {draft.candidate_questions.map((question, index) => (
              <div key={index} className="flex gap-2">
                <label htmlFor={`candidate-question-${index}`} className="sr-only">Candidate question {index + 1}</label>
                <input
                ...
                {draft.candidate_questions.length > 2 ? (
                  <button type="button" aria-label={`Remove candidate question ${index + 1}`} className="inline-flex size-11 shrink-0 items-center justify-center rounded-xl text-ink-muted hover:bg-surface-muted" onClick={() => setDraft({ ...draft, candidate_questions: draft.candidate_questions.filter((_, itemIndex) => itemIndex !== index) })}>
          ...
          <div className="mt-4 rounded-xl bg-surface-muted p-3">
            <p className="text-xs font-bold uppercase tracking-wide text-ink-muted">Suggested for this interview</p>
            <div className="mt-2 flex flex-wrap gap-2">
              {guidance.suggested_questions.map((question) => (
                <button key={question} type="button" className="rounded-lg border border-line bg-surface px-3 py-2 text-left text-xs font-semibold leading-5 text-ink hover:border-accent/40" onClick={() => addSuggestedQuestion(question)}>
                  <Plus aria-hidden="true" className="mr-1 inline size-3.5" />{question}
                </button>
              ))}
            </div>
          </div>
        ...
        {interview.status === "COMPLETED" ? (
          <section aria-labelledby="debrief-title" className="border-t border-line pt-6">
            <div className="flex items-start gap-3">
              <CheckCircle2 aria-hidden="true" className="mt-0.5 size-5 shrink-0 text-success" />
              <div><h3 id="debrief-title" className="font-bold text-ink">Post-interview debrief</h3><p className="mt-1 text-xs leading-5 text-ink-muted">Your private reflection—not employer feedback or an automated assessment.</p></div>
            </div>
            <div className="mt-4 grid gap-5 sm:grid-cols-2">
              <DebriefField label="What went well" required value={draft.debrief_went_well} onChange={(value) => setDraft({ ...draft, debrief_went_well: value })} />
              <DebriefField label="What would you improve" value={draft.debrief_improve} onChange={(value) => setDraft({ ...draft, debrief_improve: value })} />
              <DebriefField label="Signals you noticed" value={draft.debrief_signals} onChange={(value) => setDraft({ ...draft, debrief_signals: value })} />
              <DebriefField label="Concrete next step" required maxLength={500} value={draft.debrief_next_step} onChange={(value) => setDraft({ ...draft, debrief_next_step: value })} />
            </div>
          </section>
        ) : null}
      </div>
